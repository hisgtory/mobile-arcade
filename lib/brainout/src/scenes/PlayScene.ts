...
export class PlayScene extends Phaser.Scene {
  ...
  create(): void {
    // Load stage config first to get lives
    ...
    this.livesLeft = this.stageConfig.lives;
  ...
  private createChoiceButtons(puzzle: Puzzle, dpr: number): void {
    ...
    for (let i = 0; i < choices.length; i++) {
      ...
      const btn: ChoiceButton = { bg, label, option, tapCount: 0 };
      this.choiceButtons.push(btn);

      bg.on('pointerdown', () => this.handleChoiceTap(btn));
      bg.on('pointerover', () => {
        if (this.phase === GamePhase.PLAYING) bg.setFillStyle(0xF3F4F6);
      });
      bg.on('pointerout', () => {
        if (this.phase === GamePhase.PLAYING) bg.setFillStyle(0xFFFFFF);
      });
    }
  }
  ...
  private handleChoiceTap(btn: ChoiceButton): void {
    ...
    if (puzzle.interaction === 'multi-tap') {
      const targetId = puzzle.correctIds[0];
      if (btn.option.id === targetId) {
        btn.tapCount++;
        btn.label.setText(`${btn.option.label} (${btn.tapCount})`);

        const requiredTaps = this.getRequiredTaps(puzzle);
        if (btn.tapCount >= requiredTaps) {
    ...
    if (puzzle.interaction === 'sequence') {
      ...
      if (btn.option.id === expectedId) {
        ...
        this.tapCounts.set('__seq__', 0);
        this.choiceButtons.forEach(b => {
          b.bg.setFillStyle(0xFFFFFF);
          b.bg.setInteractive();
        });
        this.onWrongAnswer(btn);
      }
  ...
  private get currentPuzzle(): Puzzle | undefined {
  ...
  public useHint(): void {
    ...
    // Highlight correct answer briefly with proper interpolation
    const startColor = Phaser.Display.Color.IntegerToColor(0xFEF3C7);
      ...
      if (puzzle.correctIds.includes(btn.option.id)) {
        this.tweens.addCounter({
          ...
          onUpdate: (tween) => {
            ...
            const col = Phaser.Display.Color.Interpolate.ColorWithColor(startColor, endColor, 100, v);            if (btn.bg.active) {
              btn.bg.setFillStyle(Phaser.Display.Color.GetColor(col.r, col.g, col.b));
            }
          }
        });
      }
