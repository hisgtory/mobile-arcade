...
import { GamePhase, TILE_IMAGES, type GameConfig, type StageConfig, type CellPos, type Order } from '../types';
...
export class PlayScene extends Phaser.Scene {
  ...
  private async processMatches(matches: CellPos[][]): Promise<void> {
    ...
    for (const order of this.orders) {
      ...
      if (matched > 0) {
        order.collected = Math.min(order.collected + matched, order.target);
      }
    }
  ...
  private emitOrders(): void {
    this.game.events.emit('orders-update', {
