/**
 * PlayScene for Woodoku
 *
 * 9×9 grid with 3×3 sub-grid regions + 3 piece slots at bottom.
...
import { GamePhase, GRID_SIZE, REGION_SIZE, randomPiece, type GameConfig, type PieceShape } from '../types';

const CELL_BG_COLOR = 0xfaf3e8; // warm cream
const CELL_BORDER_COLOR = 0xd4c5a9;
const GRID_BG_COLOR = 0xf0e6d3;
const REGION_BORDER_COLOR = 0x8b7355;
...
export class PlayScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.rectangle(
      ...
      gridTotalH + 4,
      GRID_BG_COLOR,
    ).setStrokeStyle(2, REGION_BORDER_COLOR);

    // Draw 3×3 region borders
    for (let rr = 0; rr < 3; rr++) {
      for (let rc = 0; rc < 3; rc++) {
        const rx = this.gridStartX + rc * REGION_SIZE * this.cellSize;
        const ry = this.gridStartY + rr * REGION_SIZE * this.cellSize;
        const rw = REGION_SIZE * this.cellSize;
        const rh = REGION_SIZE * this.cellSize;
        this.add.rectangle(rx + rw / 2, ry + rh / 2, rw, rh)
          .setFillStyle()
          .setStrokeStyle(2, REGION_BORDER_COLOR, 0.6);
      }
    }
...
class PieceSlot {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, piece: PieceShape, cellSize: number, index: number) {
    ...
    hitArea.on('drag', (_pointer: Phaser.Input.Pointer) => {
