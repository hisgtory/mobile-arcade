/**
 * PlayScene for Blocky Quest
 *
 * 8x8 grid + 3 piece slots at bottom.
 * Drag pieces onto the grid. Full rows/cols clear.
 * Stage-based: limited piece sets, target score to clear.
 *
 * Events emitted:
 *   'score-update'    — { score }
 *   'pieces-update'   — { pieceSetsLeft }
 *   'stage-clear'     — { score }
 *   'game-over'       — { score }
 */
...
import { GamePhase, GRID_SIZE, randomPiece, type GameConfig, type StageConfig, type PieceShape } from '../types';
...
export class PlayScene extends Phaser.Scene {
  ...
  private pieceSetsLeft: number = 0;
  ...
  init(data: { stage?: number; gameConfig?: GameConfig }): void {
    ...
    this.gameConfig = data?.gameConfig ?? (this.game as any).__blockyquestConfig;
  }
  ...
  create(): void {
    ...
    this.pieceSetsLeft = this.stageConfig.pieceSets;
  ...
  private spawnPieceSlots(): void {
    ...
    for (let i = 0; i < 3; i++) {
      const piece = randomPiece(this.stageConfig.piecePoolSize);
  ...
  private afterPlace(): void {
    // Check if all 3 pieces used → spawn new set or end
    if (this.pieceSlots.every((s) => s.used)) {
      ...
      if (this.pieceSetsLeft <= 0) {
        // No more piece sets, game over
        this.gameOver();
        return;
      }
  ...
  private showComboText(lineCount: number, combo: number, score: number): void {
    ...
    const colors = [0x3b82f6, 0x22c55e, 0x8b5cf6, 0xf43f5e, 0xf59e0b];
  ...
  private stageClear(): void {
    ...
    this.game.events.emit('stage-clear', { score: this.score });
  }
  ...
  private emitPieces(): void {
    this.game.events.emit('pieces-update', { pieceSetsLeft: this.pieceSetsLeft });
  }
