...
export class Tile extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    // Forest-themed background (light green)
    this.bg = scene.add.rectangle(0, 0, size - 2, size - 2, 0xf1f8e9, 1);
    this.bg.setStrokeStyle(1, 0xc5e1a5, 0.8);
    this.add(this.bg);
  ...
  /** Highlight tile when part of a valid tappable group */
  highlight(on: boolean): void {
    if (on) {
      this.bg.setFillStyle(0xe8f5e9, 1);
      this.bg.setStrokeStyle(2, 0x66bb6a, 1);
    } else {
      this.bg.setFillStyle(0xf1f8e9, 1);
      this.bg.setStrokeStyle(1, 0xc5e1a5, 0.8);
    }
  }
