...
## ADR-018: 모노레포 의존성 버전 정책 통일 (Issue #237)

### Context
웹 패키지(`web/*`, `lib/found3-react`)는 React 18.3.x를 사용하고, RN 슈퍼앱(`rn/`)은 Expo SDK 54 요구사항으로 React 19.1.0을 사용하여 모노레포 내 버전 불일치 상태였다. 특히 `lib/found3-react`에 `@types/react: "18.2.79"` 하위 패키지 override가 존재하여 세 가지 버전이 혼재했다.

### 버전 매트릭스 (통일 전)

| 패키지 | react | react-dom | @types/react | @types/react-dom | typescript |
|--------|-------|-----------|--------------|------------------|------------|
| `rn/` | 19.1.0 (exact) | - | ^19.1.17 | - | ^5.5.0 |
| `web/found3` | ^18.3.0 | ^18.3.0 | ^18.3.0 | ^18.3.0 | ^5.5.0 |
| `web/crunch3` | ^18.3.0 | ^18.3.0 | ^18.3.0 | ^18.3.0 | ^5.5.0 |
| `web/arcade` | ^18.3.0 | ^18.3.0 | ^18.3.0 | ^18.3.0 | ^5.5.0 |
| `lib/found3-react` | ^18.3.0 | - | **18.2.79 (pin)** | - | ^5.5.0 |

### 버전 매트릭스 (통일 후)

| 패키지 | react | react-dom | @types/react | @types/react-dom | typescript |
|--------|-------|-----------|--------------|------------------|------------|
| `rn/` | 19.1.0 (exact) | - | ^19.1.0 | - | ^5.5.0 |
| `web/found3` | ^19.1.0 | ^19.1.0 | ^19.1.0 | ^19.1.0 | ^5.5.0 |
| `web/crunch3` | ^19.1.0 | ^19.1.0 | ^19.1.0 | ^19.1.0 | ^5.5.0 |
| `web/arcade` | ^19.1.0 | ^19.1.0 | ^19.1.0 | ^19.1.0 | ^5.5.0 |
| `lib/found3-react` | ^19.1.0 | - | ^19.1.0 | - | ^5.5.0 |

### Decision

1. **루트 `pnpm.overrides`** — `package.json`에 추가하여 모든 하위 패키지의 버전을 강제 통일:
   ```json
   "pnpm": {
     "overrides": {
       "react": "19.1.0",
...
2. **하위 패키지 override 제거** — `lib/found3-react`의 `pnpm.overrides` 블록 삭제.

3. **개별 패키지 선언 업데이트** — 각 `package.json`의 버전 선언을 `^19.1.0`으로 갱신하여 root overrides와 일관성 유지.

### Rationale
- `rn/`(Expo SDK 54 + RN 0.81)이 React 19를 요구하므로 React 19 기준으로 통일
- 루트 override가 있으면 새 패키지 추가 시 자동 적용 → 버전 드리프트 방지
- `lib/found3-react`의 `18.2.79` pin은 오래된 타입으로 인한 타입 오류 잠재 위험이 있어 제거

### Consequences
- 모든 패키지가 `react@19.1.0`, `react-dom@19.1.0`, `@types/react@19.1.17`, `@types/react-dom@19.2.3`으로 단일 resolve
- 새 게임/패키지 추가 시 react 버전 명시 불필요 (root override 자동 적용)
- 하위 패키지 override 금지 원칙 수립 (버전 통일성 보장)
