...
### 서바이벌 메카닉
- 각 스테이지에 **제한 시간** 존재
- 타일 매칭 성공 시 **시간 보너스** (+3초)
- 일정 시간 간격으로 **새 타일이 빈 칸에 추가** (스테이지 모드)
...
| Stage | 그림 종류 | 타일 수 | 보드 크기 | 시간(초) | 타일 추가 간격 |
|-------|-----------|---------|-----------|----------|---------------|
| 1 | 4 | 16 | 4×4 | 120 | 없음 |
| 2 | 6 | 24 | 6×4 | 120 | 없음 |
| 3 | 8 | 32 | 8×4 | 120 | 30초 |
...
> 타일 수 = 그림 종류 × 2 (항상 짝수, 모든 타일이 쌍을 이룸)
>
> 타일 추가: Stage 3부터 서바이벌 요소 시작. 빈 칸에 랜덤 쌍 추가.
...
- [ ] 기획서 작성
- [ ] 기본 타일 격자 보드 (4×4 ~ 8×6)
- [ ] 타일 쌍 매칭 + 경로 탐색 알고리즘 (BFS, 0/1/2 bend)
- [ ] 연결 선 시각화
- [ ] 게임 오버 / 클리어 판정
- [ ] 5 스테이지
- [ ] 기존 픽셀 음식 타일 에셋 재사용
