...
function AllInHoleTitleRoute() {
  ...
  return (
    <PlayLayout css={{ justifyContent: 'center', alignItems: 'center', gap: 12 }}>
      ...
      <p style={{ fontSize: 16, color: '#6B7280' }}>Swallow everything into the hole!</p>
      <button
...
function AllInHolePlaying({ stage, onClear, onGameOver }: { stage: number; onClear: (r: AllInHoleResult) => void; onGameOver: (r: AllInHoleResult) => void }) {
  const { containerRef, score, swallowed, totalObjects, timeRemainingSec, doRestart } = useAllInHoleGame({ stage, onClear, onGameOver });
  return (
    <PlayLayout>
      <AllInHoleHUD stage={stage} score={score} swallowed={swallowed} totalObjects={totalObjects} timeRemainingSec={timeRemainingSec} onRestart={doRestart} />
      <GameCanvas ref={containerRef} />
    </PlayLayout>
  );
}
