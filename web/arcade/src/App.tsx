...
function BlockyQuestTitleRoute() {
  ...
  return (
    <PlayLayout css={{ justifyContent: 'center', alignItems: 'center', gap: 12 }}>
      ...
      <p style={{ fontSize: 16, color: '#6B7280' }}>Place blocks & clear lines to complete quests!</p>
      <button
...
function BlockyQuestPlaying({ stage, onClear, onGameOver }: { stage: number; onClear: (r: BlockyQuestResult) => void; onGameOver: (r: BlockyQuestResult) => void }) {
  const { containerRef, score, pieceSetsLeft, targetScore } = useBlockyQuestGame({ stage, onClear, onGameOver });
  return (
    <PlayLayout>
      <BlockyQuestHUD stage={stage} score={score} targetScore={targetScore} pieceSetsLeft={pieceSetsLeft} />
      <GameCanvas ref={containerRef} />
    </PlayLayout>
  );
}
