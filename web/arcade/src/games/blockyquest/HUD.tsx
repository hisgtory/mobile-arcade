...
interface HUDProps {
  ...
  score: number;
  targetScore: number;
  pieceSetsLeft: number;
}

export function HUD({ stage, score, targetScore, pieceSetsLeft }: HUDProps) {
  return (
    <Container>
      <StatBlock>
      ...
      <StatBlock>
        <StatLabel>Sets Left</StatLabel>
        <StatValue css={{ color: pieceSetsLeft <= 1 ? '$rose500' : '$text' }}>
          {pieceSetsLeft}
        </StatValue>
      </StatBlock>
    </Container>
