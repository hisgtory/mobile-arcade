...
export function useGame({ stage, onClear, onGameOver }: UseGameOptions) {
  ...
  useEffect(() => {
    ...
    game.events.on('state-update', (data: { score: number; lives: number; maxLives: number; foundCount: number; totalDiffs: number; elapsedMs: number }) => {
      setScore(data.score);
      setLives(data.lives);
  ...
  return { containerRef, score, lives, maxLives, foundCount, totalDiffs, elapsedMs };
}
