...
import { createGame, destroyGame, getPlayScene } from '@arcade/lib-tangledrope';
...
export function useGame({ stage, onClear }: UseGameOptions) {
  ...
  useEffect(() => {
    ...
    game.events.on('intersections-update', (data: { intersections: number }) => {
  ...
  return { containerRef, moves, intersections, doUndo, doRestart };
}
