...
import { createGame, destroyGame, getPlayScene } from '@arcade/lib-escaperoom';
...
export function useGame({ stage, onClear }: UseGameOptions) {
  ...
  const [clues, setClues] = useState({ collected: 0, total: 0 });
  ...
  useEffect(() => {
    ...
    game.events.on('clue-update', (data: { collected: number; total: number }) => {
      setClues(data);
    });
  ...
  return { containerRef, score, clues, doRestart };
}
