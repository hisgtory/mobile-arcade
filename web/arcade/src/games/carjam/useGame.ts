...
import { createGame, destroyGame, getPlayScene } from '@arcade/lib-carjam';
...
export function useGame({ stage, onClear }: UseGameOptions) {
  ...
  const [carsRemaining, setCarsRemaining] = useState(0);
  ...
  useEffect(() => {
    ...
    game.events.on('cars-update', (data: { remaining: number; total: number }) => {
      setCarsRemaining(data.remaining);
  ...
  return { containerRef, score, moves, carsRemaining, carsTotal, doUndo, doRestart };
}
